...
contract('SupplyChain', function(accounts) {
    ...
    const productPrice = web3.utils.toWei("1000000", "wei");
    var itemState = 0;
    ...
    it("Testing smart contract function harvestItem() that allows a farmer to harvest coffee", async() => {
        ...
        assert.equal(resultBufferTwo[4], 0, 'Error: Invalid price');
    ...
    it("Testing smart contract function sellItem() that allows a farmer to sell coffee", async() => {
        ...
        assert.equal(resultBufferOne[6], originFarmLatitude, 'Error: Missing or Invalid originFarmLatitude');
        assert.equal(resultBufferOne[7], originFarmLongitude, 'Error: Missing or Invalid originFarmLongitude');
        assert.equal(resultBufferTwo[4], productPrice, 'Error: Selling price mismatch');
    ...
    it("Testing smart contract function buyItem() that allows a distributor to buy coffee", async() => {
        ...
        await supplyChain.buyItem(upc, {from: distributorID, value: 2000000000});
    ...
    it("Testing smart contract function receiveItem() that allows a retailer to mark coffee received", async() => {
        ...
        await supplyChain.receiveItem(upc, {from: retailerID, value: 2000000000});
    ...
    it("Testing smart contract function purchaseItem() that allows a consumer to purchase coffee", async() => {
        ...
        // Mark an item as Sold by calling function buyItem()
        await supplyChain.purchaseItem(upc, {from: consumerID, value: 2000000000});
    ...
    // 11th Test - Can harvest a UPC only once (because the contract logic always increments the SKU upon harvesting)
    it("Testing smart contract function harvestItem() will not allow the same UPC to be re-harvested", async() => {
        const supplyChain = await SupplyChain.deployed();
        ...
        // For the rest of this test case to work as expected, it is necessary for test case #1 to have also been called earlier in the same test run
        // The Harvested event was emitted in test case #1 and therefore eventEmitted should evaluate to true now
        assert.equal(eventEmitted, true, 'No UPC was harvested in this test run');

        // Attempt re-harvesting the same UPC
        try {
            await supplyChain.harvestItem(upc, originFarmerID, originFarmName, originFarmInformation, originFarmLatitude, originFarmLongitude, productNotes);
        } catch (error) {
            // We're expecting there to be an error because of the contract function modifier
            // Retrieve the just now saved item from blockchain by calling function fetchItem()
            const resultBufferOne = await supplyChain.fetchItemBufferOne.call(upc);
            const resultBufferTwo = await supplyChain.fetchItemBufferTwo.call(upc);

            // Verify the result set -- it should be the same as at the end of test case #8
            itemState = 7;
            assert.equal(resultBufferOne[0], sku, 'Error: Invalid item SKU');
            assert.equal(resultBufferOne[1], upc, 'Error: Invalid item UPC');
            assert.equal(resultBufferOne[2], consumerID, 'Error: Missing or Invalid ownerID');
